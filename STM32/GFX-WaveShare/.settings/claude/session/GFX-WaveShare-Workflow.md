...
**Если RST и DC тоже окажутся в порядке** — следующие подозреваемые по убыванию
вероятности:
1. Неверный контроллер экрана (не ILI9486, а другой чип из той же линейки
   Waveshare 3.5" A/B/C — они используют разные драйверы: ILI9486/ILI9481/ST7796,
   разные наборы команд)
2. Неверная полярность/логика MADCTL, VCOM, Gamma в `ILI9486_Init()` — все текущие
   значения пока не подтверждены экспериментально (см. Часть 4.1)
3. Неверная полярность подсветки (LCD_BL) — маловероятно, т.к. подсветка визуально
   включается, но само по себе это не доказывает что уровень (SET) — правильный
   активный уровень, а не случайное совпадение с дефолтным состоянием пина
