...
### 5.5 `Core/Src/MyButtonController.cpp` (⚠️ версия ниже оказалась с багом — активный уровень указан неверно, реальная финальная версия и разбор в Части 7ter)
...
## ЧАСТЬ 7ter. ПРОБЛЕМА С КНОПКОЙ K1 — НЕ РЕАГИРОВАЛА НА НАЖАТИЕ (РЕШЕНА)

### 7ter.1 Симптом

В TouchGFX Designer настроен Interaction: Trigger "Hardware button is clicked" (key = 0) → Action "Change Box1's color" (на красный, RGB 250/7/7). При нажатии физической кнопки **K1** на плате (PC13) — ничего не происходило, `box1` оставался чёрным.

Это отдельная, самостоятельная проблема, не связанная с "снегом" из Части 7/7bis — там был баг в цепочке "framebuffer → экран", здесь баг в цепочке "физическая кнопка → framework".

### 7ter.2 Диагностика — шаг за шагом

**Шаг 1. Проверили конфигурацию GPIO для PC13.**
Посмотрели `MX_GPIO_Init()` в `Core/Src/main.c` — и обнаружили, что **PC13 там вообще не упоминается**. CubeMX включает тактирование только для `GPIOH` и `GPIOE` (нужны только для дисплея), а весь код в `MyButtonController.cpp`, который читает `HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13)`, был написан "с верой", что этот пин настроен — но настроен он никогда не был. Без включенного тактирования (`__HAL_RCC_GPIOC_CLK_ENABLE()`) и без вызова `HAL_GPIO_Init()` для этого пина чтение регистра не отражает реальное состояние ножки микроконтроллера.

**Первое исправление** (частичное, само по себе не решило проблему) — добавили в `MX_GPIO_Init()`:
```c
__HAL_RCC_GPIOC_CLK_ENABLE();

GPIO_InitStruct.Pin = GPIO_PIN_13;
GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
GPIO_InitStruct.Pull = GPIO_PULLUP; // предположение: кнопка замыкает на GND
HAL_GPIO_Init(GPIOC, &GPIO_InitStruct);
```
— **Результат: кнопка всё равно не реагировала.** Значит, помимо отсутствия конфигурации, была вторая проблема.

**Шаг 2. Прямой аппаратный тест, в обход TouchGFX.**
Чтобы не гадать, работает ли сама кнопка/GPIO физически, или проблема в том, что TouchGFX не опрашивает `ButtonController` — временно вставили в главный цикл `while(1)` в `main.c` код, который напрямую отражает состояние кнопки на подсветку экрана (`LCD_BL`, PE10) — пин, который мы точно знаем рабочим и видимым (он и так уже управляет подсветкой):
```c
// ДИАГНОСТИКА: подсветка экрана гаснет, когда зажата кнопка K1, напрямую, без TouchGFX
HAL_GPIO_WritePin(LCD_BL_GPIO_Port, LCD_BL_Pin,
    HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13) == GPIO_PIN_RESET ? GPIO_PIN_SET : GPIO_PIN_RESET);
```
(Идея: `LCD_BL` активна низким уровнем — 0 = подсветка горит. Если при зажатой кнопке пин PC13 читается как RESET, то мы пишем в LCD_BL значение SET — подсветка должна погаснуть на время нажатия.)
- **Результат: подсветка вообще не реагировала на кнопку** (с настройкой `GPIO_PULLUP` из Шага 1).

**Шаг 3. Проверили противоположную полярность.**
Раз с `GPIO_PULLUP` (предположение "кнопка замыкает на GND") ничего не происходило — заподозрили, что кнопка вообще подключена наоборот: замыкает не на GND, а на VDD (3.3В). При таком монтаже внутренний pull-up "маскирует" нажатие — пин и в покое, и при нажатии читается как HIGH, разницы нет. Поменяли настройку на `GPIO_PULLDOWN` и инвертировали тестовую логику:
```c
GPIO_InitStruct.Pull = GPIO_PULLDOWN;
...
// ДИАГНОСТИКА: зеркалим сырой уровень пина, без предположений о полярности
HAL_GPIO_WritePin(LCD_BL_GPIO_Port, LCD_BL_Pin,
    HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13) == GPIO_PIN_SET ? GPIO_PIN_SET : GPIO_PIN_RESET);
```
- **Результат: подсветка стала гаснуть именно при нажатии K1!** Значит: (а) кнопка физически подключена к PC13 — пин верный; (б) кнопка активна **высоким** уровнем (при нажатии пин уходит в HIGH, замыкая на VDD), а не низким, как изначально предполагал закомментированный код в `MyButtonController.cpp` ("активный уровень LOW" — оказалось неверно).

**Побочный эффект, который сначала сбил с толку:** пока действовала настройка с `GPIO_PULLDOWN`, но СТАРАЯ (ещё не исправленная) логика в `MyButtonController::sample()` продолжала искать `GPIO_PIN_RESET` как признак "нажатия". А с pulldown состояние покоя (кнопка не нажата) — это и есть `RESET` (низкий уровень)! Получилось, что framework воспринял "кнопка в покое" как "кнопка нажата" (условие `RESET && previousState==0` выполнилось один раз сразу), `box1` окрасился в красный один раз — а настоящее нажатие (переход в HIGH) код воспринимал как "отпускание" (сбрасывал `previousState`). Отсюда и наблюдение "экран стал красным, а при нажатии картинка пропадает" — картинка не "пропадала", просто гасла подсветка (это наш диагностический код), а красный цвет — это случайное однократное срабатывание инвертированной логики Interaction.

### 7ter.3 Итоговая причина (коротко)

**Две независимые проблемы, обе на уровне GPIO-конфигурации, никак не связанные с TouchGFX-фреймворком:**
1. Пин PC13 (кнопка K1) вообще не был сконфигурирован в CubeMX-коде — ни тактирование `GPIOC`, ни режим входа. Читать его было в принципе нельзя.
2. После того как пин настроили, выяснилось, что физически кнопка K1 на этой плате подключена **активным высоким уровнем** (замыкает на VDD), а не низким, как предполагал изначальный (написанный "на глаз", без проверки на реальном железе) код в `MyButtonController.cpp`.

### 7ter.4 Финальные изменения в коде

#### Изменение 1: `Core/Src/main.c`, функция `MX_GPIO_Init()` — настройка PC13

**Добавлено** (в секцию `USER CODE BEGIN MX_GPIO_Init_2`):
```c
...
```
**Зачем:**
- `__HAL_RCC_GPIOC_CLK_ENABLE()` — включает тактирование порта GPIOC. На STM32 каждый периферийный блок (включая GPIO-порты) подключён к шине через собственный тактовый сигнал, и по умолчанию после сброса большинство из них отключены для экономии энергии. Без этого вызова регистры порта C физически не обновляются, и любое чтение/запись пина даёт непредсказуемый результат.
- `GPIO_MODE_INPUT` — переводит пин в режим цифрового входа (по умолчанию после сброса пины МК часто уже в режиме входа, но явно указать — надёжнее и понятнее).
- `GPIO_PULLDOWN` — включает внутренний резистор подтяжки к GND. Это то, что гарантирует: когда кнопка НЕ нажата (и физически никуда не подключена, кроме подтяжки на VDD через саму кнопку при нажатии), пин стабильно читается как LOW, а не "болтается" в неопределённом состоянии (флоатинг), которое могло бы случайно читаться как то LOW, то HIGH из-за наводок.

#### Изменение 2: `Core/Src/MyButtonController.cpp` — инверсия полярности

**Было:**
```cpp
bool MyButtonController::sample(uint8_t& key)
{
    // Проверяем физическую кнопку K1 (PC13), активный уровень LOW
    if ((HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13) == GPIO_PIN_RESET) && previousState == 0x00)
    {
        previousState = 0xFF;
        key = 0;
...
**Зачем:** логика функции не изменилась структурно — это всё тот же классический паттерн "детектор переднего фронта с защёлкой" (edge detector with latch):
- Первый `if` смотрит: "пин показывает состояние нажатия, И мы ещё не зарегистрировали это нажатие (`previousState == 0`)?" — если да, взводим защёлку (`previousState = 0xFF`), сообщаем framework "нажата кнопка с кодом 0" и возвращаем `true`.
- Пока кнопка удерживается, защёлка не даёт послать событие повторно на каждый вызов `sample()` (иначе framework решил бы, что кнопку нажимают десятки раз в секунду).
- Второй `if` смотрит: "пин вернулся в состояние покоя?" — если да, сбрасываем защёлку, чтобы следующее нажатие снова сработало.
- Единственное, что было неправильным — САМИ уровни, которые считаются "нажато" и "отпущено", были переставлены местами относительно того, что реально происходит на пине. Поменяли `GPIO_PIN_RESET`↔`GPIO_PIN_SET` в обоих `if`, и логика стала соответствовать реальному железу.

### 7ter.5 Урок

Комментарий в коде ("активный уровень LOW") — это не факт, а **недоказанное предположение**, которое туда попало на этапе написания кода "по аналогии" с тем, как чаще всего бывает на подобных платах (кнопка на GND — действительно самый частый вариант). Но конкретно эта плата оказалась исключением. Мораль: электрические предположения про незнакомую плату (полярность кнопок, активные уровни LED и т.п.) нужно **проверять экспериментально** (как в Шаге 2-3 выше — отразить сырой сигнал на что-то заведомо видимое), а не доверять комментарию/интуиции, особенно на платах вроде WeAct, где документация и реальная схема уже не раз расходились (см. Часть 1.2 и Часть 9.1).

---

## ЧАСТЬ 8. ПЛАН НА БУДУЩЕЕ (после решения текущей проблемы)
